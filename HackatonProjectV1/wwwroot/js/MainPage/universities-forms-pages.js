document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. URL'DEN ÜNİVERSİTE İSMİNİ AL VE YAZDIR (YENİ EKLENDİ) ---
    const urlParams = new URLSearchParams(window.location.search);
    const uniName = urlParams.get('uniName');
    const uniMainBtn = document.getElementById('btn-main-university');

    if (uniName && uniMainBtn) {
        // Butonun içindeki yazıyı değiştir
        uniMainBtn.textContent = uniName;
        // Mouse üzerine gelince tam ismini göster (Tooltip gibi)
        uniMainBtn.title = uniName;
    }
    ...
    // Buton ID'si 'btn-main-university' olarak güncellendiği için burada da güncelliyoruz
    if (uniMainBtn) {
        uniMainBtn.addEventListener('click', () => {
            allTabs.forEach(t => t.classList.remove('active'));
            document.querySelectorAll('.dropdown-btn').forEach(btn => btn.classList.remove('active'));
            uniMainBtn.classList.add('active');
